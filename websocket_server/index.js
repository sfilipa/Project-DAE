...
io.on("connection", (socket) => {
  ...
  socket.on(
    "repairerAssignedWithoutNeedForApproval",
    function (repairerUsername) {
  ...
  // Expert Disapproved Repairer
  socket.on("occurrenceRepairerDisapproved", function (clientUsername) {
    occurrenceDisapproved(socket, clientUsername);
  });

  // Repairer Started Occurrence
  socket.on("repairerStartedOccurrence", function (client) {
    socket.to(client).emit("repairerStartedOccurrence");
  });

  // Repairer Failed Occurrence
  socket.on("repairerFailedOccurrence", function (client) {
    socket.to(client).emit("repairerFailedOccurrence");
  });

  // Repairer Finish Occurrence
  socket.on("repairerFinishedOccurrence", function (client) {
    socket.to(client).emit("repairerFinishedOccurrence");
  });
});
